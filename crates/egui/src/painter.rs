...
impl Painter {
    ...
    #[inline]
    fn fonts_mut<R>(&self, writer: impl FnOnce(&mut Option<Fonts>) -> R) -> R {
        self.ctx.fonts_mut(writer)
    }
...
impl Painter {
    #[inline]
    fn paint_list_mut<R>(&self, writer: impl FnOnce(&mut PaintList) -> R) -> R {
    ...
    pub fn add(&self, shape: impl Into<Shape>) -> ShapeIdx {
        if self.fade_to_color == Some(Color32::TRANSPARENT) {
            self.paint_list_mut(|l| l.add(self.clip_rect, Shape::Noop))
        } else {
            let mut shape = shape.into();
            self.transform_shape(&mut shape);
            self.paint_list_mut(|l| l.add(self.clip_rect, shape))
        }
    }
    ...
    pub fn extend(&self, mut shapes: Vec<Shape>) {
        ...
        if !shapes.is_empty() {
            ...
            self.paint_list_mut(|l| l.extend(self.clip_rect, shapes));
        }
    }
    ...
    pub fn set(&self, idx: ShapeIdx, shape: impl Into<Shape>) {
        ...
        let mut shape = shape.into();
        self.transform_shape(&mut shape);
        self.paint_list_mut(|l| l.set(idx, self.clip_rect, shape));
    }
}
